...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, tsify_next::Tsify)]
...
/// The lowering front-half, exposed for the live
/// [`DogwoodAuthorizer`](crate::live::DogwoodAuthorizer): it needs the
/// [`LoweredPolicySet`] itself (to own an `Authorizer` across calls) rather
/// than a report. Same lowering as every other op.
pub fn lower_for_authorizer(
    source: &str,
    schema: &SchemaInputs,
    action_schema: &str,
) -> Result<LoweredPolicySet, OpError> {
    lower_internal(source, schema, action_schema)
}
